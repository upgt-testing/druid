 ...
 */
class QueryVirtualStorageTest_RestartInjected extends EmbeddedClusterTestBase
{
  ...
  {
    RestartFramework.at("before_query_too_much_data")
        .on(cluster)
        .restart("historical")
        .withIndex(0)
    ...
    RestartFramework.at("after_query_error_verified")
        .on(cluster)
        .restart("broker")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  void testQueryPartials()
  {
    ...
    RestartFramework.at("after_query_interval_1")
        .on(cluster)
        .restart("broker")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_query_interval_3")
        .on(cluster)
        .restart("historical")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_all_interval_queries")
        .on(cluster)
        .restart("coordinator")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_random_queries_loop")
        .on(cluster)
        .restart("broker")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
