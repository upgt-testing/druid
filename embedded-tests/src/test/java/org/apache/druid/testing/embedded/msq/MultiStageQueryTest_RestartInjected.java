...
public class MultiStageQueryTest_RestartInjected extends EmbeddedClusterTestBase
{
  ...
  public void testMsqIngestionAndQuerying()
  {
    ...
    RestartFramework.at("after_msq_task_complete")
        .on(cluster)
        .restart("indexer")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
  ...
  public void testExport()
  {
    ...
    RestartFramework.at("after_export_task_submit")
        .on(cluster)
        .restart("overlord")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
        .execute();
    cluster.callApi().waitForTaskToSucceed(taskStatus.getTaskId(), overlord.latchableEmitter());
    RestartFramework.at("after_export_task_complete")
        .on(cluster)
        .restart("indexer")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("before_export_query")
        .on(cluster)
        .restart("broker")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_export_verification")
        .on(cluster)
        .restart("overlord")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
