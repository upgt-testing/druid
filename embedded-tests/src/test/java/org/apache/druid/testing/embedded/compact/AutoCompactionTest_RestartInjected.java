 ...
 */
public class AutoCompactionTest_RestartInjected extends CompactionTestBase
{
  private static final Logger LOG = new Logger(AutoCompactionTest_RestartInjected.class);
  ...
  public void testAutoCompactionRowWithMetricAndRowWithoutMetricShouldPreserveExistingMetricsUsingAggregatorWithDifferentReturnType() throws Exception
  {
    ...
    RestartFramework.at("after_data_load_1")
        .on(cluster)
        .restart("indexer")
        .withIndex(0)
    ...
    RestartFramework.at("after_data_load_2")
        .on(cluster)
        .restart("indexer")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    try (final Closeable ignored = unloader(fullDatasourceName)) {
      ...
      RestartFramework.at("after_compaction_config_submit")
          .on(cluster)
          .restart("coordinator")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_compaction_trigger_1")
          .on(cluster)
          .restart("overlord")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      forceTriggerAutoCompaction(1);
      RestartFramework.at("after_compaction_trigger_2")
          .on(cluster)
          .restart("overlord")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testAutoCompactionDutySubmitAndVerifyCompaction() throws Exception
  {
    ...
    try (final Closeable ignored = unloader(fullDatasourceName)) {
      ...
      RestartFramework.at("after_compaction_config_submit_2")
          .on(cluster)
          .restart("coordinator")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testAutoCompactionDutyCanUpdateCompactionConfig(CompactionEngine engine) throws Exception
  {
    ...
    try (final Closeable ignored = unloader(fullDatasourceName)) {
      ...
      RestartFramework.at("after_compaction_config_submit_1")
          .on(cluster)
          .restart("coordinator")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
      ...
      if (engine == CompactionEngine.NATIVE) {
        ...
        RestartFramework.at("after_compaction_config_submit_3")
            .on(cluster)
            .restart("coordinator")
            .withIndex(0)
            .withMode(RestartMode.GRACEFUL)
      ...
      RestartFramework.at("after_compaction_config_submit_4")
          .on(cluster)
          .restart("coordinator")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
          .execute();
      forceTriggerAutoCompaction(2);
      RestartFramework.at("after_compaction_trigger_3")
          .on(cluster)
          .restart("overlord")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
  ...
  public void testAutoCompactionDutyWithSegmentGranularityAndWithDropExistingTrue(CompactionEngine engine) throws Exception
  {
    ...
    try (final Closeable ignored = unloader(fullDatasourceName)) {
      ...
      List<TaskStatusPlus> compactTasksBefore = getCompleteTasksForDataSource(fullDatasourceName);
      forceTriggerAutoCompaction(2);
      RestartFramework.at("after_compaction_trigger_4")
          .on(cluster)
          .restart("overlord")
          .withIndex(0)
          .withMode(RestartMode.GRACEFUL)
