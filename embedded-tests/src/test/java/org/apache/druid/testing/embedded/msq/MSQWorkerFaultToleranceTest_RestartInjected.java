 ...
 */
public class MSQWorkerFaultToleranceTest_RestartInjected extends EmbeddedClusterTestBase
{
  ...
  public void test_cancelledWorker_isRetried_ifFaultToleranceIsEnabled() throws Exception
  {
    ...
    RestartFramework.at("after_fault_tolerant_task_submit")
        .on(cluster)
        .restart("overlord")
        .withIndex(0)
    ...
    faultyIndexer.start();
    RestartFramework.at("after_faulty_indexer_start")
        .on(cluster)
        .restart("coordinator")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_functional_indexer_start")
        .on(cluster)
        .restart("overlord")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    faultyIndexer.stop();
    RestartFramework.at("after_worker_task_failed")
        .on(cluster)
        .restart("overlord")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
    ...
    RestartFramework.at("after_controller_task_success")
        .on(cluster)
        .restart("coordinator")
        .withIndex(0)
        .withMode(RestartMode.GRACEFUL)
